...
use image::{imageops, Rgba, ImageBuffer, RgbaImage};
use log;
use pgn_reader::{BufferedReader, SanPlus, Skip, Visitor};
use shakmaty::{Chess, Position, Move, Color, Role, Square, Setup, Pieces};
use usvg;
use resvg;
...
impl BoardDrawer {
    ...
    pub fn draw_move_on_board(&mut self, _move: &Move, color: Color, board: &mut RgbaImage) {
        ...
        match _move {
            Move::Normal { role, from, capture, to, promotion } => {
                ...
                let blank_to_square = if capture.is_some() {
                    true
                } else {
                    false
                };
            ...
            Move::Castle { king, rook }=> {
                ...
                let offset = if rook.file() > king.file() {
                    1
                } else {
                    -1
                };
    ...
    pub fn get_piece_rgba_image(&mut self, color: Color, role: &Role, width: u32, height: u32) -> &RgbaImage {
        let p = self.piece_map.entry(format!("{}/{}_{}.svg", self.image_path, color.char(), role.char())).or_insert_with_key(|key| {
...
impl PGNGiffer {
    pub fn new(image_path: &str, board_size: u32, output_path: &str, ms_delay: u16, dark: [u8; 4], light: [u8; 4]) -> Self {
...
impl Visitor for PGNGiffer {
    ...
    fn san(&mut self, san_plus: SanPlus) {
        if let Ok(m) = san_plus.san.to_move(&self.position) {
            ...
            self.drawer.draw_move_on_board(&m, self.position.turn(), &mut board);
...
impl Chess2Gif {
        ...
        T: Into<OsString> + Clone,
    {
        ...
        let size = u32::from_str_radix(
            matches.value_of("size").expect("Size must be defined"),
            10
        ).unwrap();
        let pgn = matches.value_of("PGN").expect("PGN is required");
        let pieces_path = matches.value_of("pieces-path").expect("Path to pieces must be defined");
        let output = matches.value_of("output").expect("Output must be defined");

        let dark: [u8; 4] = clap::values_t_or_exit!(matches, "dark", u8).try_into().unwrap();
        let light: [u8; 4] = clap::values_t_or_exit!(matches, "light", u8).try_into().unwrap();
